...
import { Box, Button, TextField, Divider, Stack, Typography } from '@mui/material'
...
const MediaReview = ({ reviews, media, mediaType }) => {
  ...
  const onRemoved = reviewId => {
    if (listReviews.findIndex(review => review.id === reviewId) !== -1) {
      const newListReviews = [...listReviews].filter(review => review.id !== reviewId)
